...
def test_the_first_spelling_in_the_data_is_the_one_reported():
    """Deterministic: the answer must not depend on the hash seed. Deriving
    the value from a set made the same file report 'MUS' or 'mus' between
    runs (caught when the suite moved, 2026-09-25)."""
    assert single_value(pd.Series(["mus", "MUS"])).value == "mus"
    assert single_value(pd.Series(["MUS", "mus"])).value == "MUS"
    for _ in range(50):
        assert single_value(pd.Series([" MUS", "mus", "MUS "])).value == "MUS"
