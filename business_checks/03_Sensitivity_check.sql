-- =============================================================================
-- BUSINESS CHECK — Sensitivity    ·    run in each DB, export to Excel, compare
...
-- COB    = COB date (YYYYMMDD)        ·    USD amount column: MEASURE_VALUE_USD
...
SELECT COUNT(*) AS ROWS, SUM(MEASURE_VALUE_USD) AS TOTAL_USD
FROM FACT.SENSITIVITY_MEASURES_ADJUSTMENT WHERE ADJUSTMENT_ID = $ADJ_ID;                 -- total
SELECT * FROM FACT.SENSITIVITY_MEASURES_ADJUSTMENT WHERE ADJUSTMENT_ID = $ADJ_ID ORDER BY COBID;   -- detail

-- FACT.SENSITIVITY_MEASURES
--   ORIGINAL values, before any adjustment. Key: COBID
SELECT COUNT(*) AS ROWS, SUM(MEASURE_VALUE_USD) AS TOTAL_USD
FROM FACT.SENSITIVITY_MEASURES WHERE COBID = $COB;

-- FACT.SENSITIVITY_MEASURES_ADJUSTED
--   FINAL values = original + all adjustments (what the reports show). Key: COBID
SELECT COUNT(*) AS ROWS, SUM(MEASURE_VALUE_USD) AS TOTAL_USD
FROM FACT.SENSITIVITY_MEASURES_ADJUSTED WHERE COBID = $COB;

-- FACT.SENSITIVITY_MEASURES_ADJUSTMENT_SUMMARY
--   per-COB summary of the adjustment delta. Key: COBID
SELECT * FROM FACT.SENSITIVITY_MEASURES_ADJUSTMENT_SUMMARY WHERE COBID = $COB;
