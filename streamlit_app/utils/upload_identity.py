...
def single_value(series, numeric: bool = False) -> SingleValue:
    ...
    if numeric:
        ...
        distinct = sorted({v for v in _clean(pd.Series(series))},
                          key=lambda x: x.upper())
        # Case-insensitive identity: 'Mus' and 'MUS' are one entity.
        folded = {}
        for v in distinct:
            folded.setdefault(v.upper(), v)
        distinct = list(folded.values())
    if not distinct:
        return SingleValue(None, [], "none")
    if len(distinct) > 1:
        return SingleValue(None, distinct, "many")
