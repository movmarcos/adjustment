...
The legacy render_grid/df_from_grid HTML helpers were deleted on 2026-09-22
(unused everywhere — see the full-review Batch 4), so their tests went with
them; the _grids() assertions below stay, as the guarantee that these grids
are native and emit no HTML table at all.
"""
import os
import sys
...
def test_activity_grid_is_fixed_height_dataframe(harness):
    ...
    # scroll; every other page uses render_df_table.
    md, dfs, _ = harness
    src = pd.DataFrame([{
