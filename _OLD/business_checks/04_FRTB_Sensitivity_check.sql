-- =============================================================================
-- BUSINESS CHECK — FRTB    ·    run in each DB, export to Excel, compare
...
-- COB    = COB date (YYYYMMDD)        ·    USD amount column: AMOUNT_IN_USD
...
SELECT COUNT(*) AS ROWS, SUM(AMOUNT_IN_USD) AS TOTAL_USD
FROM FACT.FRTBSA_SENSITIVITY_MEASURES_ADJUSTMENT WHERE ADJUSTMENT_ID = $ADJ_ID;                 -- total
SELECT * FROM FACT.FRTBSA_SENSITIVITY_MEASURES_ADJUSTMENT WHERE ADJUSTMENT_ID = $ADJ_ID ORDER BY COBID;   -- detail

-- FACT.FRTBSA_SENSITIVITY_MEASURES
--   ORIGINAL values, before any adjustment. Key: COBID
SELECT COUNT(*) AS ROWS, SUM(AMOUNT_IN_USD) AS TOTAL_USD
FROM FACT.FRTBSA_SENSITIVITY_MEASURES WHERE COBID = $COB;

-- FACT.FRTBSA_SENSITIVITY_MEASURES_ADJUSTED
--   FINAL values = original + all adjustments (what the reports show). Key: COBID
SELECT COUNT(*) AS ROWS, SUM(AMOUNT_IN_USD) AS TOTAL_USD
FROM FACT.FRTBSA_SENSITIVITY_MEASURES_ADJUSTED WHERE COBID = $COB;
