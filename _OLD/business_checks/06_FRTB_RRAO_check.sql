-- =============================================================================
-- BUSINESS CHECK — FRTB RRAO    ·    run in each DB, export to Excel, compare
...
-- COB    = COB date (YYYYMMDD)        ·    USD amount column: NOTIONAL_AMOUNT_USD
...
SELECT COUNT(*) AS ROWS, SUM(NOTIONAL_AMOUNT_USD) AS TOTAL_USD
FROM FACT.FRTBSA_RRAO_MEASURES_ADJUSTMENT WHERE ADJUSTMENT_ID = $ADJ_ID;                 -- total
SELECT * FROM FACT.FRTBSA_RRAO_MEASURES_ADJUSTMENT WHERE ADJUSTMENT_ID = $ADJ_ID ORDER BY COBID;   -- detail

-- FACT.FRTBSA_RRAO_MEASURES
--   ORIGINAL values, before any adjustment. Key: COBID
SELECT COUNT(*) AS ROWS, SUM(NOTIONAL_AMOUNT_USD) AS TOTAL_USD
FROM FACT.FRTBSA_RRAO_MEASURES WHERE COBID = $COB;

-- FACT.FRTBSA_RRAO_MEASURES_ADJUSTED
--   FINAL values = original + all adjustments (what the reports show). Key: COBID
SELECT COUNT(*) AS ROWS, SUM(NOTIONAL_AMOUNT_USD) AS TOTAL_USD
FROM FACT.FRTBSA_RRAO_MEASURES_ADJUSTED WHERE COBID = $COB;
