...
CREATE OR ALTER TABLE ADJUSTMENT_APP.ADJ_DIRECT_STAGE (
    ...
    UNDERLYING_TENOR_CODE VARCHAR(10),
...
USING (
    ...
    -- Stress — template aligned 2026-08 with the users' current file
    -- (log.txt): COBID now IN the file (validated app-side against the page
    -- COB, never staged), BOOK_CODE mandatory, canonical Snowflake names +
    -- their legacy CamelCase headers as aliases. SourceCOBID / ScaleFactor
    -- are deliberately NOT accepted (meaningless for Direct — the
    -- exact-template rule rejects files still carrying them).
    SELECT 'Stress' AS PROCESS_TYPE, c.ACCEPTED_NAME, c.STAGE_COLUMN, c.IS_REQUIRED, c.DISPLAY_ORDER
    FROM (
        SELECT 'COBID' ACCEPTED_NAME,   'COBID' STAGE_COLUMN, TRUE IS_REQUIRED, 5 DISPLAY_ORDER UNION ALL
        SELECT 'COB',               'COBID',              FALSE, NULL UNION ALL
        SELECT 'ENTITY_CODE',       'ENTITY_CODE',        TRUE,  10 UNION ALL
        SELECT 'ENTITY',            'ENTITY_CODE',        FALSE, NULL UNION ALL
        SELECT 'ENTITYCODE',        'ENTITY_CODE',        FALSE, NULL UNION ALL
        SELECT 'SOURCE_SYSTEM_CODE','SOURCE_SYSTEM_CODE', FALSE, 20 UNION ALL
        SELECT 'SOURCESYSTEMCODE',  'SOURCE_SYSTEM_CODE', FALSE, NULL UNION ALL
        SELECT 'DEPARTMENT_CODE',   'DEPARTMENT_CODE',    FALSE, NULL UNION ALL
        SELECT 'DEPARTMENT',        'DEPARTMENT_CODE',    FALSE, NULL UNION ALL
        SELECT 'BOOK_CODE',         'BOOK_CODE',          TRUE,  30 UNION ALL
        SELECT 'BOOK',              'BOOK_CODE',          FALSE, NULL UNION ALL
        SELECT 'BOOKCODE',          'BOOK_CODE',          FALSE, NULL UNION ALL
        SELECT 'CURRENCY_CODE',     'CURRENCY_CODE',      FALSE, 40 UNION ALL
        SELECT 'CURRENCY',          'CURRENCY_CODE',      FALSE, NULL UNION ALL
        SELECT 'CURRENCYCODE',      'CURRENCY_CODE',      FALSE, NULL UNION ALL
        SELECT 'INSTRUMENT_CODE',   'INSTRUMENT_CODE',    FALSE, 50 UNION ALL
        SELECT 'INSTRUMENT',        'INSTRUMENT_CODE',    FALSE, NULL UNION ALL
        SELECT 'INSTRUMENTCODE',    'INSTRUMENT_CODE',    FALSE, NULL UNION ALL
        SELECT 'VALUE_USD',         'VALUE_USD',          TRUE,  60 UNION ALL
        SELECT 'VALUE',             'VALUE_USD',          FALSE, NULL UNION ALL
        SELECT 'AMOUNT',            'VALUE_USD',          FALSE, NULL UNION ALL
        SELECT 'ADJUSTMENT_VALUE',  'VALUE_USD',          FALSE, NULL UNION ALL
        SELECT 'ADJUSTMENTVALUESIMULATIONPLINUSD', 'VALUE_USD', FALSE, NULL UNION ALL
        SELECT 'REASON',            'REASON',             FALSE, 70 UNION ALL
        SELECT 'ADJUSTMENT_DETAIL', 'REASON',             FALSE, NULL UNION ALL
        SELECT 'ADJUSTMENTDETAIL',  'REASON',             FALSE, NULL UNION ALL
        SELECT 'TRADE_TYPOLOGY',    'TRADE_TYPOLOGY',     FALSE, 80 UNION ALL
        SELECT 'TRADETYPOLOGY',     'TRADE_TYPOLOGY',     FALSE, NULL UNION ALL
        SELECT 'STRATEGY',          'STRATEGY',           FALSE, NULL UNION ALL
        SELECT 'TRADE_CODE',        'TRADE_CODE',         FALSE, 90 UNION ALL
        SELECT 'TRADE',             'TRADE_CODE',         FALSE, NULL UNION ALL
        SELECT 'TRADECODE',         'TRADE_CODE',         FALSE, NULL UNION ALL
        SELECT 'SIMULATION_NAME',   'SIMULATION_NAME',    FALSE, 100 UNION ALL
        SELECT 'SIMULATION',        'SIMULATION_NAME',    FALSE, NULL UNION ALL
        SELECT 'SIMULATIONNAME',    'SIMULATION_NAME',    FALSE, NULL UNION ALL
        SELECT 'SIMULATION_SOURCE', 'SIMULATION_SOURCE',  FALSE, 110 UNION ALL
        SELECT 'SIMULATIONSOURCE',  'SIMULATION_SOURCE',  FALSE, NULL UNION ALL
        SELECT 'PRODUCT_CATEGORY_ATTRIBUTES','PRODUCT_CATEGORY_ATTRIBUTES', FALSE, 120 UNION ALL
        SELECT 'PRODUCTCATEGORYATTRIBUTES',  'PRODUCT_CATEGORY_ATTRIBUTES', FALSE, NULL UNION ALL
        SELECT 'PCA',                'PRODUCT_CATEGORY_ATTRIBUTES', FALSE, NULL
    ) c

    UNION ALL

    -- Sensitivity — template aligned 2026-08 with the users' current file
    -- (log.txt): COBID in the file (validated vs the page COB, not staged),
    -- BOOK_CODE mandatory, UNDERLYING_TENOR_CODE added (their file carries
    -- it; header + engine already supported it), legacy CamelCase aliases.
    -- SourceCOBID / ScaleFactor deliberately NOT accepted.
    ...
    FROM (
        ...
        SELECT 'TENORCODE',         'TENOR_CODE',         FALSE, NULL UNION ALL
        SELECT 'UNDERLYING_TENOR_CODE','UNDERLYING_TENOR_CODE', FALSE, 45 UNION ALL
        SELECT 'UNDERLYINGTENORCODE',  'UNDERLYING_TENOR_CODE', FALSE, NULL UNION ALL
        SELECT 'UNDERLYING_TENOR',     'UNDERLYING_TENOR_CODE', FALSE, NULL UNION ALL
        ...
        SELECT 'CURVECODE',         'CURVE_CODE',         FALSE, NULL UNION ALL
        ...
        SELECT 'MEASURETYPECODE',   'MEASURE_TYPE_CODE',  FALSE, NULL UNION ALL
        SELECT 'VALUE_USD',         'VALUE_USD',          TRUE,  90 UNION ALL
        SELECT 'VALUE',             'VALUE_USD',          FALSE, NULL UNION ALL
        SELECT 'AMOUNT',            'VALUE_USD',          FALSE, NULL UNION ALL
        SELECT 'ADJUSTMENT_VALUE',  'VALUE_USD',          FALSE, NULL UNION ALL
        SELECT 'ADJUSTMENTVALUEINUSD', 'VALUE_USD',       FALSE, NULL UNION ALL
