...
-- ═══════════════════════════════════════════════════════════════════════════
-- 10. VW_SQL_LOG — Readable view over ADJ_SQL_LOG
--
-- Most-recent-first feed of every dynamic statement the engine ran, joined to
-- ADJ_HEADER (via RUN_LOG_ID) so you can see which adjustment(s) a statement
-- belonged to. To trace one run end-to-end:
--   SELECT * FROM ADJUSTMENT_APP.VW_SQL_LOG WHERE RUN_LOG_ID = <id> ORDER BY SEQ_NO;
-- ═══════════════════════════════════════════════════════════════════════════

CREATE OR REPLACE VIEW ADJUSTMENT_APP.VW_SQL_LOG
    COMMENT = 'Readable feed of ADJ_SQL_LOG: every executed statement with status, rows, duration, and the adjustments it belonged to.'
AS
SELECT
    l.LOG_ID,
    l.LOGGED_AT,
    ...
    l.SQL_TEXT,
    l.USERNAME,
    -- DIMENSION.ADJUSTMENT IDs touched by this run (for cross-reference)
    (SELECT LISTAGG(DISTINCT h.DIMENSION_ADJ_ID, ', ')
       FROM ADJUSTMENT_APP.ADJ_HEADER h
      WHERE h.RUN_LOG_ID = l.RUN_LOG_ID) AS DIMENSION_ADJ_IDS
FROM ADJUSTMENT_APP.ADJ_SQL_LOG l;
