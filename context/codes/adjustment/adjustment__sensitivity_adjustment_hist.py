...
def model(dbt, session):
  ...
  dbt.config(
      ...
      python_version="3.11",
      tags=["sensitivity_adjustment", "adjustment", "publish_mrm_rm_adjustment"]
  )

  # References to the base models
  df_adjustments_model = dbt.ref('adjustment__adjustments_base_sensitivity')
  df_adjustments_model = dbt.ref('fact__sensitivity_measures_adjustment')
